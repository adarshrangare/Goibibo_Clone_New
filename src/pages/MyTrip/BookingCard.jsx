...
const BookingCard = ({ type }) => {
  ...
  return (
    <div
      className={`shrink-0 min-h-fit border rounded-lg transition-all overflow-hidden ${
        current > dayjs(start_date)
          ? "border-green-300 "
          : " shadow-md hover:scale-[1.005] hover:shadow-xl"
      }`}
    >
      <div
      ...
      >
        {current > dayjs(start_date) && (
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-md  text-green font-bold text-4xl bg-green-50  px-4 py-2   text-green-100 text-center w-full h-full"></div>
        )}
        <div
          className={`min-h-6 border-b grid gap-x-2  px-4 pt-2   overflow-auto ${
            current > dayjs(start_date) ? "opacity-60" : ""
          } ${
            booking_type == "hotel"
              ? "bg-amber-50"
              : booking_type == "flight"
        ...
        >
          <span className=" max-w-fit col-start-1 col-end-1 capitalize text-slate-500 font-medium text-sm md:text-base">
            {booking_type}
          </span>
          <span className="text-wrap font-medium text-slate-700 col-start-2 col-end-4 text-center text-sm md:text-base ">
            {name}
            <br />
            {booking_type == "flight" && (
        ...
        <div className={`${current > dayjs(start_date) ? "opacity-50" : ""}`}>
            ...
            <>
              <div className="w-full flex text-center items-stretch justify-around text-sm md:text-base  ">
                ...
                <div className="dateOfJourney self-center">
                  <div className="max-md:hidden text-sm text-gray-400">
                    Journey Date
                  </div>
                  <div className="font-medium">
                    {dayjs(start_date).format("ddd, DD-MMM-YYYY")}
                  </div>
                </div>
