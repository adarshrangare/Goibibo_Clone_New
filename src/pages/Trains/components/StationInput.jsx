...
const StationInput = ({
  ...
  const [trainStationList, setTrainStationList] = useState(trainStations);
  ...
  function handleInput(inputText) {
    ...
    if (inputText.trim().length > 0) {
      ...
      // setTrainStationList(trainStations)
    } else {
    ...
    <div className={`inputBox flex relative p-0 hover:cursor-pointer`}>
      ...
      <label
        ...
        className={`absolute select-none top-[2px] left-6 px-1 rounded bg-[#fff]  font-medium leading-[18px] text-sm ${
          error ? "text-red-500" : "text-[rgb(119,119,119)] hover:cursor-pointer"
        }  ${labelClass}`}
      >
        {label ? label : "Input"}
      </label>
